...
- [x] Task 9: サンプル（SampleBank・WAV・`n`/begin/end/speed・ピッチ付き）+ sound 解決（波形→サンプル）※ティアA ※同梱は Sonic Pi CC0（Dirt-Samples ではない）
- [x] Task 10: 曲ファイルフォーマット `song.rs`（bpm ヘッダ + 名前付きトラック + コメント）
- [x] Task 11: Deck（トラック集合のスケジューリング + ボイスプール）
- [x] Task 12: Engine 統合（Command enum, バー境界 pending 適用, 単一デッキで鳴る）
- [x] Task 13: バークオンタイズ検証（変更がバー頭でのみ反映されることの自動テスト）
- [x] Task 14: デュアルデッキ + Mixer 層（A/B フェーダー・EQ/フィルター・等パワー xfade）
- [x] Task 15: 曲ファイルウォッチャ（notify → バー境界リロード）
- [x] Task 16: REPL（デッキ/ミキサー操作コマンド一式）
- [x] Task 17: HTTP API（REST + SSE 状態ストリーム）— 既定ポート **17878**（`--port` / `STRUDEL_API_PORT`）
- [x] Task 18: MCP server（stdio→HTTP ブリッジ, ツール7種; 手書き JSON-RPC、rmcp 非依存）
- [x] Task 19: エラーハンドリング（パース失敗で現行曲継続、API/REPL 両面の報告）
...
- [x] Task 26（任意）: **ミニ記法ライブハイライト**（曲ソース表示 + 再生中 atom の ANSI 強調）
