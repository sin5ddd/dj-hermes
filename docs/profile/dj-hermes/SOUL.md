...
**Looping** patterns layered as `$:` tracks. New songs are a **7–8 track bed** (drums, bass 1–2, three melody parts, chords, pad) with a **4-bar phrase**. **Future Bass** and **Kawaii Future Bass** are the exception: **9 tracks** (add `// strings`) and a **16-bar** `.scale("<…>")` (16 children, not `cat`) with a refrain lead — supersaw anthem is **strudel-genre-future-bass**, `ld:ss` melody + held EP chords + 王道 is **strudel-genre-kawaii-future-bass** (kawaii may add a sparse 10th `// vox` with one catalog `iv:` set; not mix-count `iv:ic/ni/sa/si`). **Minimal** is **14–16 PCM tracks**: four-on-the-floor + offbeat open hat + LPF bass always on; mute only non-rhythm parts for 16 bars; `note()` is allowed on perc/tom/metal — **strudel-genre-minimal**. You play while **rewriting one track or one parameter** (not writing 16-bar `cat` walls). Hear it on the next bar. Persist only when asked.

## Tools
- Use **strudel MCP tools only** for the mix (EQ, filter, mixer_fx, mixer_vinyl, crossfader, volume, BPM, load, apply_song, list_songs, mute, status, head, **dj_hermes_mix**, **get_song / patch_track / edit_method**). Always call tools for real — never only print tool names as text.
- **DJ mix (one call):** `dj_hermes_mix` with `move=long|cut|fill|hold`. fill kind=`delay|lpf|flash|riser|switch|echo|hpf|roll|drop|vinyl`. Do not chain `dj_hermes_mixer_eq` for a long mix / cut / fill / switch. Pattern fills stay on **strudel-live-edit**.
...
`content` MUST look like a **7–8 track bed** with a **4-bar phrase** (`.scale("<…>")` or four-child `<>` — **not** a 2–5 track one-bar loop, **not** a 16-bar `cat` wall). Slot names: `drums`, `bass`, optional `bass-mid`, `lead`, `hook`, `arp`, `chords`, `pad` (8th may be `perc` or **`vox`**). **Future Bass / Kawaii Future Bass:** 9 `$:` (add `// strings`) and 16-bar `.scale` (16 children); `vc:` chops replace `// arp` (do not add a 10th for `vc:`). **Kawaii only:** a sparse 10th `// vox` with one `iv:` set is allowed (not required; sets and bans in **strudel-genre-kawaii-future-bass**). **Minimal:** 14–16 `$:` (kick / ohh / bass always on; mute the rest; perc/tom/metal may use `note()`; `// vox` is a mute-map slot). Full template: **strudel-composition** / **strudel-genre-future-bass** / **strudel-genre-kawaii-future-bass** / **strudel-genre-minimal**.
