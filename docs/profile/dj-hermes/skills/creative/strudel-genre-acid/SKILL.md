...
description: >-
    ...
    16ths as the hook, named chromatic pitches (no .scale on the 303),
    gain around 0.3. Other slots (drums kit, bass filter/LFO, lead,
...
Keep patterned `.lpf`, `.lpq(14)`, `.lpenv(3)`, lp attack/decay/sustain, `.cut(1)`, amp ADSR, and **`.gain(0.3)`** on `// hook`. Do not add a second 303. Write the 303 as **16 named pitches** (`c2`, `eb2`, `e2`, `f#2` …). **No `.scale` on that `$:`** — a TB-303 sequencer is chromatic steps, not diatonic degrees. Integer degrees plus `.scale("C2:minor")` lock the line to one mode; skip both. Other pitched tracks (bass / lead / chords / pad) still use the 4-bar `.scale("<…>")`.
...
| `note("c2 …")` 16 named tokens, **no `.scale`**            | Monophonic 16ths on **hook**. Chromatic names (`eb2` `e2` `f#2` `b1`). Not integer degrees. Not `c3'min` (suffix = root only). |
| `.gain(0.3)`                                               | 303 level. `0.44` is too hot against the kit.                                                                                  |
...
| `.s("sawtooth").adsr("0.001:0.1:0.12:0.04")`               | Amp **gate** next to `.s()`. **Not** after `.gain()`. **Not** a substitute for `lpenv`.                                        |
...
**Why the 303 has no `.scale`.** A TB-303 sequencer stores chromatic steps (and octave jumps), not scale degrees. Named notes (`c2`, `eb2`, `e2`, `f#2`) already are Hz; `.scale("C2:minor")` is unused and would only tempt integer degrees back onto the line. The 16-step `.lpf("260 …")` already walks the bar — do not add `.scale("<…>")` on the same `$:` (that would retune the 16ths every bar while the accent bases stayed locked). Bass / lead / chords / pad take the 4-bar `<>`.
...
| Another 303 riff    | rewrite the 16 named notes (semitone neighbors, octave jumps). Keep 16 tokens                                                                                |
...
| Key jump            | fifth up or fourth down on **bass / lead / chords / pad** — not C D Eb E in song order. The 303 stays named notes (transpose the names, or write a new riff) |
...
1. The 303 sound is **`.lpenv` + high `.lpq` + low/mid `.lpf` base**. Amp ADSR is only the gate: **`.s("sawtooth").adsr(…)`**, then filters, then **`.gain(0.3)`**. Do not chain `.attack` / `.adsr` after `.gain()`.
...
6. Do not put `.scale` on the 303 line (not a scalar, not `"<…>"`). Named chromatic notes only.
7. Do not ship 02–09 as the same 303 bed with only a rising key. Jump the other tracks’ keys. Change the sub filter, the drum kit, and (for a new apply) the 16 named notes.

## Checklist

- [ ] `setcpm(130/4)` + **7 `$:`** (drums, bass, hook=303, lead, arp, chords, pad). 7 is OK
- [ ] 16 named chromatic notes (no `.scale`), lpf pattern, `lpq(14)`, `lpenv(3)`, lp ADSR, `cut(1)`, amp ADSR, **`.gain(0.3)`** on hook
...
- Put `.scale` (scalar or `"<…>"`) or integer degrees on the 303 hook — hardware 303 is chromatic named notes.
