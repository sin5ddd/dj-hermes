...
| arp | melody + `note()` | `plk:hd`, `plk:aj`, `plk:lp` | `perc:`, a third mid oscillator |
| chords | `[0,4]` | `plk:s5`, `ep:mt` | `triangle`, maj7 city-pop |
| pad | **C2** + `.adsr` | `pf:fo`, `dr:rd`, `pf:ff`+`note("0")` | C4, ADSR なし, music box, Rhodes, `ps:mx` |
...
Lead and hook share a 4-bar `.scale("<C4:minor C4:minor G4:phrygian C4:minor>")` and rest on different slots. Bass / bass-mid keep scalar `C2:minor` so the Reese does not jump octaves with the melody progression. Arp is a melody instrument (`plk:` + `note()`), not perc. Pad is **C2** + `.adsr`. Lead PCM takes `.adsr` after `.s()`.
...
- [ ] 4-bar phrase on lead / hook / arp / chords / pad. Melody call-and-response. Lead `.adsr`. **Pad at C2**. Arp is melody, not `perc:`
