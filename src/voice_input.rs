//! Live TUI voice input: cpal capture → cloud STT (xAI) → text for Hermes.
//!
...
pub struct SttConfig {
    pub api_key: String,
    /// e.g. `https://api.x.ai/v1` (endpoint is `{base}/stt`).
    pub base_url: String,
    pub language: String,
...
impl SttConfig {
    /// Build from env. Returns `None` when no API key is configured.
    ///
    /// Keys (first non-empty wins): `STRUDEL_STT_API_KEY`, `XAI_API_KEY`.
    pub fn from_env() -> Option<Self> {
        let api_key =
            env_nonempty("STRUDEL_STT_API_KEY").or_else(|| env_nonempty("XAI_API_KEY"))?;
...
impl VoiceHandle {
    ...
    pub fn start(config: SttConfig) -> Result<Self, String> {
        ...
        thread::Builder::new()
            ...
            .spawn(move || worker_loop(config, cmd_rx, event_tx, rec_flag, stt_flag))
    ...
    /// Toggle recording (start if idle, stop+STT if recording).
    pub fn toggle(&self) {
...
fn worker_loop(
    ...
    loop {
        if active.is_some() {
            let should_stop = match cmd_rx.recv_timeout(Duration::from_millis(40)) {
...
fn start_capture(config: &SttConfig) -> Result<ActiveCapture, String> {
    let host = cpal::default_host();
    let device = host
        .default_input_device()
        .ok_or_else(|| "no default input device".to_string())?;
    ...
    let max_samples = ((config.max_recording_secs + 0.5) * sample_rate as f32).ceil() as usize
...
fn stop_capture(active: &mut Option<ActiveCapture>, recording_flag: &AtomicBool) -> CaptureResult {
...
/// POST `{base}/stt` (xAI Grok STT) with multipart WAV.
pub fn transcribe_wav_bytes(config: &SttConfig, wav: &[u8]) -> Result<String, String> {
    ...
    let url = format!("{}/stt", config.base_url.trim_end_matches('/'));
...
mod tests {
    ...
    fn stt_config_defaults_shape() {
        let cfg = SttConfig {
            ...
            min_recording_secs: 0.4,
        };
        assert!(cfg.base_url.ends_with("/v1"));
